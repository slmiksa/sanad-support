...
export const Route = createFileRoute("/api/public/admin-password/set")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const { user_id, password } = (await request.json()) as {
            user_id?: string;
            password?: string;
          };
          ...
          if (!isSuper) {
            ...
            if (!ok) return json({ error: "لا تملك صلاحية تغيير كلمة المرور لهذه العضوية" }, 403);
          }
